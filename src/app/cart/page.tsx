...
import { formatPrice, getAuthToken, getAuthUser } from "@/lib/utils";
...
export default function CartPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderRes = await ordersApi.create({ items: orderItems }, token) as any;
      
      if (!orderRes.success) {
        throw new Error(orderRes.message || "Failed to secure rental order");
      }
      ...
      if (!paymentRes.success) {
        throw new Error(paymentRes.message || "Payment authorization failed");
      }
